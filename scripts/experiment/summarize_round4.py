...
def summarize(qos, loss_pct, direction, rows, samples, rng):
...
def main():
    ...
    for path in args.csv_paths:
        try:
            qos, loss_pct, direction, rows = read_condition(path)
    ...
    condition_rows = [
        summarize(qos, loss_pct, direction, rows, args.bootstrap_samples, rng)
        for (direction, qos, loss_pct), rows in sorted(groups.items(), key=lambda item: (item[0][0], item[0][2], item[0][1]))
    ]

    effects = []
    for direction in sorted({key[0] for key in groups}):
        for loss_pct in sorted({loss for group_direction, _, loss in groups if group_direction == direction}):
            reliable = groups.get((direction, "reliable", loss_pct))
            best_effort = groups.get((direction, "best_effort", loss_pct))
    ...
    lines = [
        ...
        "## Conditions", "",
        "| Direction | QoS | Loss (%) | N | Delivery mean % [95% CI] | Delivery median % | Delivery SD % | RTT mean ms [95% CI] | RTT median ms | RTT run-mean p95 ms | RTT SD ms |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in condition_rows:
        lines.append(
            "| {direction} | {qos} | {loss_pct} | {n} | {delivery_mean_pct:.3f} [{delivery_mean_ci_low_pct:.3f}, {delivery_mean_ci_high_pct:.3f}] | {delivery_median_pct:.3f} | {delivery_sd_pct:.3f} | {rtt_mean_ms:.3f} [{rtt_mean_ci_low_ms:.3f}, {rtt_mean_ci_high_ms:.3f}] | {rtt_median_ms:.3f} | {rtt_run_mean_p95_ms:.3f} | {rtt_sd_ms:.3f} |".format(**row)
        )
    lines.extend(["", "## QoS Effects (RELIABLE minus BEST_EFFORT)", ""])
    if effects:
        lines.extend([
            "| Direction | Loss (%) | Reliable N | Best Effort N | Delivery difference pp [95% CI] | RTT mean difference ms [95% CI] |",
            "| --- | ---: | ---: | ---: | ---: | ---: |",
        ])
        for row in effects:
            lines.append(
                f"| {row['direction']} | {row['loss_pct']} | {row['reliable_n']} | {row['best_effort_n']} | "
